...
function ListItBuffer(opt) {
    ...
    if(opt.header != null && !Array.isArray(opt.header)) {
        throw new Error([
            "The opt.header must be an array,",
            `but ${typeof opt.header} was specified.`,
            `header: ${JSON.stringify(opt.header)}`
        ].join(" "));
    }
...
debug(`COLORS:${JSON.stringify(Object.keys(COLORS))}`);
...
ListItBuffer.prototype.applyHeaderStyleOption = function(line) {
    ...
    if(this.opt.headerColor) {
        if( !(this.opt.headerColor in COLORS) ) {
            throw new Error(`Invalid color ${
                JSON.stringify(this.opt.headerColor)
            } was specified. Available`);
        } else {
 ...
 * Set column header.
 * 
 * @param {Array<any>} header A header row
