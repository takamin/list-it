(function() {
    ...
    privates.ListItBuffer.prototype.d = function(data) {
        if(arguments.length > 1) {
            ...
            } else if(privates.ListItBuffer.isObjectArray(data)) {
    ...
    privates.ListItBuffer.isObjectArray = function(arr) {
        var allElementIsObject = true;
        var ctor = arr[0].constructor;
        arr.forEach(function(element) {
    ...
    function shortenAsPossible(arr) {
        var shorten = null;
        var max = Math.max.apply(null, arr.map(function(item){return item.length;}));
        var len = 1;
        while(shorten == null && len < max) {
            shorten = (function(truncated) {
                for(var i = 0; i < truncated.length; i++) {
                    for(var j = i + 1; j < truncated.length; j++) {
                ...
                return truncated;
            }(arr.map(function(item){ return item.substr(0, len); })));
            len++;
        }
        if(shorten == null) {
            shorten = keys;
